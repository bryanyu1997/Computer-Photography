...
def get_kernel(gau_N=7, gau_std=1.2, mv_x=0, mv_y=0):
    ...
    c_X,c_Y = (-1.5 + 4.0 * mv_y, -1.5 + 4.0 * mv_x)
    w_X,w_Y = np.mgrid[-(gau_N//2):(gau_N//2)+1:1, -(gau_N//2):(gau_N//2)+1:1]
    gau_kernel = np.exp(-(((c_X - w_X)**2 + (c_Y - w_Y)**2) / (2*(gau_std**2))))
...
def solve(imgs, filters, lamb):
    ...
    for i, img in enumerate(imgs):
        if i == 0: data = norm1(subsample(conv(filters[i], x, dims=2), (4, 4, 1)) - img)
        else: data = norm1(subsample(conv(filters[i], x, dims=2), (4, 4, 1)) - img) + data
    
    prob = Problem(data + lamb*group_norm1(grad(x, dims=2), [3])) 
    # solve problem
...
if __name__ == '__main__':
    ...
    imageio.imwrite('./result/zebra_test_eight_lambda0001.png', img_out)
    # check answer
    img_urs_path = './result/zebra_test_eight_lambda0001.png'
